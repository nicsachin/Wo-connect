import React from "react";

const IconRefetch = () => {
  return (
    <svg
      width="16"
      height="16"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M9 12C9 13.66 10.34 15 12 15C13.66 15 15 13.66 15 12C15 10.34 13.66 9 12 9C10.34 9 9 10.34 9 12Z"
        fill="black"
        fill-opacity="0.54"
      />
      <path
        d="M8 10V8H5.09C6.47 5.61 9.05 4 12 4C15.72 4 18.85 6.56 19.74 10H21.8C20.87 5.44 16.84 2 12 2C8.73 2 5.82 3.58 4 6.01V4H2V10H8Z"
        fill="black"
        fill-opacity="0.54"
      />
      <path
        d="M16.0002 14V16H18.9102C17.5302 18.39 14.9502 20 12.0002 20C8.2802 20 5.1502 17.44 4.2602 14H2.2002C3.1302 18.56 7.1602 22 12.0002 22C15.2702 22 18.1802 20.42 20.0002 17.99V20H22.0002V14H16.0002Z"
        fill="black"
        fill-opacity="0.54"
      />
    </svg>
  );
};

export default IconRefetch;
